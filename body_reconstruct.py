...
from utils import project_to_3d, lift2d_keypoints_to_3d, project_to_3d_gpu
...
class BodyReconstructionPipeline:
    ...
    def _estimate_depth_and_fov(self, image_path):
        ...
        print(f" processed image shape: {processed_image.shape}, dtype: {processed_image.dtype}, min: {processed_image.min()}, max: {processed_image.max()}, device: {self.device} ")
        ...
        cam_intrinsics = self.fov_estimator.get_cam_intrinsics(img=image)
    ...
    def infer_pose(self, processed_image):
        return infer_pose(processed_image)
    ...
    def _create_point_cloud(self, depth_map, rgb_image, sam_mask=None, clean_point_cloud=False, use_gpu=False):
        """Step 4: Create 3D point cloud from depth map."""
        print("\n[4/5] Creating 3D point cloud...")
        
        if use_gpu:
            points, colors = project_to_3d_gpu(
                depth_map=depth_map,
                img_rgb=rgb_image,
        ...
        if clean_point_cloud:
            ...
            labels = np.array(pcd.cluster_dbscan(eps=0.05, min_points=40, print_progress=False))
