...
class NLFSMPLFitter:
    ...
    def project_mesh_on_image(
        ...
        if "vertices" in params:
            ...
            print(f" vertices shape from params: {vertices.shape} ")
            vertices = vertices[0]
        else:
            with torch.no_grad():
    ...
    def export_mesh(self, output_path: str, params: Optional[Dict[str, torch.Tensor]] = None) -> None:
        ...
        print(f" Exporting mesh with {verts.shape} vertices and {faces.shape} faces ")
        self.fitted_params = original
        trimesh.Trimesh(vertices=verts.squeeze(0), faces=faces).export(output_path)
