...
def project_to_3d(depth_map, img_rgb, camera_intrinsics, sam_mask=None):
    # Mask valid pixels
    mask = (depth_map > 0.1) & (depth_map < 20000)
    ...
    # scaled x = s*x >>>> (u - cx) * (s * z )/ (fx)
    # scaled y = s*y >>>> (v - cy) * (s * z )/ (fy)
    # scaled z = s*z >>> s * z
...
def project_to_3d_gpu(depth_map, img_rgb, camera_intrinsics, sam_mask=None):
    # Mask valid pixels
    mask = (depth_map > 0.1) & (depth_map < 20000)
    height, width = depth_map.shape
    u,v = torch.meshgrid(torch.arange(width, device=depth_map.device), torch.arange(height, device=depth_map.device), indexing='xy')
    if camera_intrinsics.ndim == 3:
        camera_intrinsics = camera_intrinsics[0]
    ...
    if sam_mask is not None:
        sam_mask = sam_mask.float() > 0.5 # shape (H, W)
        mask = sam_mask[0]
